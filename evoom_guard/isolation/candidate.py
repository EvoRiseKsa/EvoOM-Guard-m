...
# ─────────────────────────────────────────────────────────────────────────────
"""Typed preparation of the boundary used to run a black-box candidate.
...
def _run_docker_control_default(
...
class CandidateRunner:
    ...
    def _docker_control(
        ...
        """Run one bounded Docker control command through the default kernel."""

        return _run_docker_control_default(command, timeout=timeout)
    ...
    def _image_digest(image: str) -> str | None:
        inspected = inspect_docker_image(
            image,
            control_runner=_run_docker_control_default,
            timeout=_DOCKER_CONTROL_TIMEOUT_SECONDS,
        )
