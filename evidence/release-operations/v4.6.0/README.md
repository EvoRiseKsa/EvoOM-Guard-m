# v4.6.0 publication-authority retirement

`KEY_RETIREMENT.json` is the canonical, separately signed post-ledger receipt
for the temporary publication authority named by the immutable `v4.6.0`
...
- repository deploy key ID `160156962`; and
...
pinned `v4.6.0` ledger public key:

```powershell
python -I <trusted-parent>\tools\ci\validate_release_ledger_v2.py validate-retirement `
  evidence\release-ledgers\v4.6.0 `
  evidence\release-operations\v4.6.0\KEY_RETIREMENT.json `
  evidence\release-operations\v4.6.0\KEY_RETIREMENT.json.sig `
  --trusted-ledger-pub <independent-v4.6.0-ledger-public-key> `
