...
| Release-bound gVisor observation | One same-owner private run used the exact published `v4.5.0` zipapp on a GitHub-hosted Linux runner, selected checksum-pinned `runsc` with network `none`, passed 9/9 required isolation probes, accepted the honest 2/2 case, and rejected the deliberately broken 0/2 case. The selected raw bytes and closed public record are retained under [`evidence/runtime-observations/v4.5.0-gvisor-31298956172`](../evidence/runtime-observations/v4.5.0-gvisor-31298956172). | It is not independent, production, field-efficacy, dedicated-host, hostile-host, multi-host, non-forking-ledger, Firecracker, or VM evidence. GitHub Actions was only the temporary transfer carrier; the checked-in subset is the durable record. Nonzero launcher receipts are not an audited exact call count. |
...
bounded and now-piloted artifact contract is specified in
...
- a dedicated-host, hostile-host, Firecracker/VM, multi-host, or externally
  operated runtime-boundary result. The retained same-owner gVisor observation
  does not close any of those gates.
