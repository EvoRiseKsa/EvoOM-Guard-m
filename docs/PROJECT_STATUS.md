...
| Agent Change Admission V1 | The published v4.3.0 bootstrap separates an untrusted proposal from a signed exact-change authorization, raw-Git binding derivation, and a Trusted Finalizer `ALLOW`. The public same-owner pilot admitted one bounded change, rejected an ignored tracked path before signing, and reproduced the same exact binding under a fresh run identity. | It is not an enabled merge gate, does not heal or judge code beyond the configured pack, is idempotent rather than single-use, and has no independent validation or general hostile-runner proof. |
