...
| --- | --- | --- | --- |
| [`EvoOM-Guard-m`](https://github.com/EvoRiseKsa/EvoOM-Guard-m) | Authoritative source-available CLI, Action, releases, threat model, and security policy. | The source declares the pre-release v4.3.0 candidate while [`v4.2.0`](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases/tag/v4.2.0) remains the latest published immutable consumer release. See [release status](RELEASE_STATUS.md). Consumers must inspect [Releases](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases) before adopting a different version pin. | External adoption, independent security review, or universal correctness. |
...
| [`evoom-guard-release-source-v2-pilot`](https://github.com/EvoRiseKsa/evoom-guard-release-source-v2-pilot) | Non-production protected-main Release Source Admission V2 evidence using the immutable `v4.1.0` runtime. | Round 1 binds source `af8e4592ef5572acfe2ea295c435eed6a8e122fc` to A/B/C attempts `29896945747/1`, `29896982146/1`, and `29897001564/1`; protected C produced `SEALED/ALLOW`, detached D produced `VERIFIED/ALLOW`, and the exact live/detached negatives are separated from unexecuted cases in [`ROUND1_EVIDENCE.md`](https://github.com/EvoRiseKsa/evoom-guard-release-source-v2-pilot/blob/main/docs/ROUND1_EVIDENCE.md). | Artifact, package, release, publication, deployment, production-gate, or independent-review authorization. |
| [`evoom-guard-agent-change-pilot`](https://github.com/EvoRiseKsa/evoom-guard-agent-change-pilot) | Public bounded consumer pilot for the unreleased Agent Change Admission V1 candidate. | The fixed record in [`PILOT_RESULTS.md`](https://github.com/EvoRiseKsa/evoom-guard-agent-change-pilot/blob/main/PILOT_RESULTS.md) covers permitted run `29983466826`, ignored tracked-path rejection `29983731021`, and exact-change replay `29983835620`. Retained positive/replay bundles verified offline; the negative produced no signed authorization or final bundle. | A v4.3 release, independent validation, production merge gate, arbitrary hostile-code isolation, patch correctness outside the pack, or single-use authorization. |
...
| Release Artifact Admission V1 | The published `v4.2.0` bootstrap implements a separately keyed `.raae` contract that re-verifies an exact `.rsae`, binds one external regular artifact to protected E/F workflow blobs and run identities, freshly constrains its GitHub Artifact Attestation, and supports detached offline verification. | It has no live E/F/G pilot evidence. It does not authorize publication or deployment and does not prove reproducibility, OCI/registry provenance, production readiness, or independent review. |
| Agent Change Admission V1 candidate | The v4.3.0 source candidate separates an untrusted proposal from a signed exact-change authorization, raw-Git binding derivation, and a Trusted Finalizer `ALLOW`. The public same-owner pilot admitted one bounded change, rejected an ignored tracked path before signing, and reproduced the same exact binding under a fresh run identity. | It is not yet published, is not an enabled merge gate, does not heal or judge code beyond the configured pack, is idempotent rather than single-use, and has no independent validation or general hostile-runner proof. |
...
published immutable [`v4.2.0`](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases/tag/v4.2.0)
...
The source tree declares the pre-release version `4.3.0`; the latest published
immutable consumer release remains `4.2.0`. The project cannot
