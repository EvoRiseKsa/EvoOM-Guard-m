...
| --- | --- | --- | --- |
| [`EvoOM-Guard-m`](https://github.com/EvoRiseKsa/EvoOM-Guard-m) | Authoritative source-available CLI, Action, releases, threat model, and security policy. | The repository publishes [`v4.2.0`](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases/tag/v4.2.0), the latest published immutable consumer release. See [release status](RELEASE_STATUS.md). Consumers must inspect [Releases](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases) before adopting a different version pin. | External adoption, independent security review, or universal correctness. |
...
| Agent Change Admission V1 candidate | Separates an untrusted proposal from a signed exact-change authorization, raw-Git binding derivation, and a Trusted Finalizer `ALLOW`. The public same-owner pilot admitted one bounded change, rejected an ignored tracked path before signing, and reproduced the same exact binding under a fresh run identity. | It is unreleased, is not an enabled merge gate, does not heal or judge code beyond the configured pack, is idempotent rather than single-use, and has no independent validation or general hostile-runner proof. |
...
The source tree and latest published immutable consumer release are `4.2.0`.
The project cannot
