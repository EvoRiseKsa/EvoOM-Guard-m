...
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.5.1   # a release tag; @<sha> is strictest, @main is latest
```
...
```bash
pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.5.1"   # a release tag — recommended
...
> pip URL:
> - **`@v3.5.1`** — a release tag. The recommended pin and the right choice for
...
`EvoRiseKsa/EvoOM-Guard-m@v3.5.1`. Copy [`examples/evoguard.yml`](../examples/evoguard.yml) to
`.github/workflows/evoguard.yml` in the repo you want to protect:

```yaml
- uses: actions/checkout@9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0 # v7
  with: { fetch-depth: 0 }            # Guard needs the base commit to diff
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.5.1   # pin a release (@<sha> strictest, @main latest)
  with:
...
- run: pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.5.1"   # see Install; @<sha> strictest for CI
- run: |
