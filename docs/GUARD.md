...
> **Release availability.** [`v3.8.0`](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases/tag/v3.8.0)
...
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.8.0   # published release; @<sha> is strictest, @main is latest
```
...
```bash
pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.8.0"   # published release
...
> pip URL:
> - **`@v3.8.0`** — the current published release tag. The recommended pin and the right choice for
...
the published `EvoRiseKsa/EvoOM-Guard-m@v3.8.0` release. Copy [`examples/evoguard.yml`](../examples/evoguard.yml) to
`.github/workflows/evoguard.yml` in the repo you want to protect:

```yaml
- uses: actions/checkout@9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0 # v7
  with: { fetch-depth: 0 }            # Guard needs the base commit to diff
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.8.0   # published release (@<sha> strictest, @main latest)
  with:
...
- run: pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.8.0"   # published release; @<sha> strictest for CI
- run: |
