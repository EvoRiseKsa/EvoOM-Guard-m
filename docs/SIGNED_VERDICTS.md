...
Requires the `sign` extra (the core gate stays stdlib-only). `v4.0.0` is the
published immutable GitHub Release:

```bash
pip install "evoom-guard[sign] @ git+https://github.com/EvoRiseKsa/EvoOM-Guard-m@v4.0.0"
```
