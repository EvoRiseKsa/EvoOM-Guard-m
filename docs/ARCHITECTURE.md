...
| `verifiers/repo_verifier.py` | **The compatibility orchestrator.** Coordinates repository admission/materialization, verifier-pack pre/post snapshot identity, sticky evidence, phase composition/final projection, and workspace cleanup. It injects live runtime-identity providers into the bounded continuity owner. For container runs it resolves a fresh canonical immutable image ID per verification and keeps that pin context-local across setup, suite, and pack phases. |
...
| `verifiers/repo_pack.py` | Typed verifier-pack owner. It executes host/docker/gVisor branches through injected live services, freezes terminal process evidence, and separately interprets judge-owned JUnit only after the orchestrator verifies the pack snapshot and the continuity owner verifies the candidate runtime again. It does not own pack admission/identity, runtime continuity, sticky evidence, composition, final projection, or cleanup. |
