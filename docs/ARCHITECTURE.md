...
| `admission/decision_envelope.py` | Unreleased, library-only Admission Decision Envelope V1 wire owner. It validates and canonicalizes one closed in-toto Statement projection for the Agent Change `ALLOW` profile. Structural inspection is non-authoritative; every external-action flag is fixed to false. |
