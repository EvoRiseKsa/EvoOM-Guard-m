...
| `verifiers/record_report.py` / `verifiers/record_isolation.py` / `verifiers/record_policy.py` / `verifiers/record_policy_types.py` / `verifiers/record_coverage_types.py` | Explicit verifier-layer owners for the stable record-check envelope/schema-support pin, isolation-parity checks, independently re-derived operating-profile semantics, and immutable ordered effective-policy and diff-coverage type/shape projections. `record_policy.py` deliberately does not import the producer's policy predicate. The two type projections perform no I/O, cleanup, process execution, hashing, report mutation, or verdict decision; the public facade retains contract selection, profile decisions, coverage-threshold semantics, and report ordering. |
