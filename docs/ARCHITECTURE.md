...
| `integrations/guard_output.py` | Typed, stdlib-only owner of Markdown projection plus JSON/SARIF publication. It consumes a structural result view, makes no verdict decision, and receives live badge, verdict, version, JSON-dumper, and SARIF-converter providers from `guard.py` so public signatures, bytes, and historical lookup positions remain unchanged. |
