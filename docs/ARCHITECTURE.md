...
| `execution/process.py` | Typed generic bounded-process requests/results, shared output capture, timeouts, and native process-tree cleanup. Cancellation preserves the exact active `BaseException`; every generic runner abort-cleanup stage that raises or is not positively proven is attached as a bounded secondary note, including a machine-readable Python 3.10 fallback. This scope does not cover the specialized judge lifecycle or the raw-Git/GitHub-Attestation process owners, which retain separate lifecycle contracts. |
