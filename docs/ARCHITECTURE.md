...
| `guard.py` | **Producer compatibility facade.** `guard()` / `guard_from_diff()` / `candidate_from_dirs()`, live effect wiring, the distinct black-box path, public `GuardResult`, and unchanged Markdown / JSON / SARIF output facades. Repo-native post-decision sequencing delegates to `application/repo_finalization.py`, pristine-baseline execution delegates through `_run_baseline_suite` to `verifiers/repo_baseline.py`, and output projection/publication delegates to `integrations/guard_output.py`; established schema-1.11 constants remain re-exported. |
