...
| `cli/` | The `evo-guard` command package. `cli/__init__.py` remains the public compatibility facade and dispatch surface. Typed stdlib-only owners now contain the orchestration for `guard`, diagnostic/version/pack inspection, the Agent Change family, the Trusted Finalizer family, and the record family (`verify-verdict`, `verify-record`, `bundle-evidence`, `finalize-record`, `verify-bundle`). The facade retains dependency lookup timing, trusted `.evoguard.json` loading, flag/config precedence, initialization, and the still-pending admission command families. |
