...
| `domain/blast_radius.py` | Dependency-free, additive `EVOGUARD_BLAST_RADIUS_V2` materialized-change contract and deterministic advisory scorer. It accepts only explicit validated add/modify/delete/rename/copy/mode records, canonicalizes exact JSON bytes, rejects raw diff and ambiguous paths fail-closed, and does not reinterpret V1 or signed verdict schemas 1.11/1.12. |
...
| `policy/preflight.py` | Read-only, effect-injected readiness analysis for one resolved Guard policy. It diagnoses command normalization, launcher/container availability, and bounded runtime-write risks without applying a patch, running candidate code, weakening runtime identity, or issuing a verdict. |
...
| `execution/judge_environment.py` | Phase-specific setup/suite/pack scratch environments owned by the judgment workspace and located outside the candidate copy. They redirect common home/temp/cache paths, including an explicit phase-private Go build cache required when Windows user-profile variables are absent, without introducing runtime-identity exclusions or claiming a host filesystem sandbox. |
...
| `patchmin.py` | Pure, model-free helpers: delta-debugging (`minimize_patch`) plus the frozen V1 `blast_radius_score`; the historical `risk_score` name remains an identity alias for compatibility. Additive materialized V2 is owned by `domain/blast_radius.py`. |
...
| `verifiers/record_report.py` / `verifiers/record_isolation.py` / `verifiers/record_policy.py` / `verifiers/record_policy_types.py` / `verifiers/record_coverage_types.py` / `verifiers/record_baseline_types.py` / `verifiers/record_envelope_types.py` | Explicit verifier-layer owners for the stable record-check envelope/schema-support pin, isolation-parity checks, independently re-derived operating-profile semantics, and immutable ordered effective-policy, diff-coverage, baseline, and top-level envelope-type projections. `record_policy.py` deliberately does not import the producer's policy predicate. The four type projections perform no I/O, cleanup, process execution, hashing, report mutation, or verdict decision; the public facade retains contract selection, profile decisions, coverage-threshold and baseline policy semantics, envelope check ordering, and all verdict authority. |
...
| `finalizer/git_command.py` | Cohesive, effect-injected owner of the bounded raw-Git subprocess lifecycle. It builds the shell-free command and scrubbed Git environment; pinned mode uses the closed environment. It drains both bounded pipes concurrently, enforces the query deadline, and independently proves process-tree then reader cleanup. Cancellation preserves the exact primary `BaseException`; only exact `True` is cleanup proof and every false or raised secondary outcome becomes an ordered bounded diagnostic. It imports no compatibility facade and does not own object interpretation, verdicts, schemas, signing, GitHub Attestation, or admission. |
| `finalizer_derivation.py` | No-checkout raw-Git object reader and canonical `EVOGUARD_FINALIZER_GIT_BINDINGS_V1` derivation for candidate text, ordered deletions, effective policy, and verifier-pack identity. It compares those results with an untrusted verdict before finalizer signing. The historical `_run_git_command` path remains a branch-free compatibility facade that snapshots its established monkeypatch providers and delegates the specialized subprocess lifecycle to `finalizer/git_command.py`; normal bytes, errors, cleanup order, limits, and executable-pin behavior remain unchanged. |
| `trusted_finalizer.py` | Trusted Finalizer handoff/sealing owner. High-assurance bundles automatically bind the exact canonical raw-Git derivation under a reserved signed material role; strict verification requires one such material and rechecks its verdict/source/context relation. Explicit `*_without_derivation` functions retain lower-assurance historical compatibility and are not admission APIs. |
...
| `change_attempt_observation.py` | Deterministic advisory projection of one authenticated Trusted Finalizer `ALLOW` or `DENY`. Its historical V1 input deliberately retains handoff-only verification and makes no raw-Git derivation claim. It preserves closed source/outcome/assurance identities, treats all five evidence channels as one correlated source, exports no raw code/log/policy content, and grants no admission or external-action authority. |
| `maintenance_bindings.py` | Additive V2 finalizer-layer contract for a maintenance source. It keeps trusted workflow commit/tree/path/blob, frozen maintenance base commit/tree, target source commit/tree, upstream run identity, and policy/pack/control-tool material roles separate. It performs closed-world validation/canonicalization only; it does not read Git, call GitHub, sign, publish, or admit. |
| `release_source_finalizer_v2.py` | Additive deny-only V2 release-source finalizer manifest and distinct signature domain. It consumes the separated bindings without changing the published V1 byte contract and grants no release or publication authority. |
| `release_source_producer_receipt_v2.py` | Additive V2 producer-receipt claim whose subject contains the maintenance base/target while the producer workflow remains bound to the trusted default-branch commit/tree and an explicit upstream run/attempt. It is an attestation subject, not admission. |
...
| `admission/artifact_provider_v3.py` | `v4.6.0` library-only provider adapter for one canonical digest-qualified public GHCR subject. It requires one exact GitHub Artifact Attestation result for a direct same-revision branch build and exact builder run/attempt, relates it to external Trusted Finalizer repository/head context, retains canonical provider evidence, and uses unchanged V2 to sign the subject/receipt/finalizer relation. Retained verification is byte continuity; only explicit fresh reverification contacts the provider. Isolated mode inherits no ambient Docker config, so the registry-auth path is not yet demonstrated. It has no CLI, protected reference workflow, live OCI pilot, registry-retention, publication, deployment, reproducibility, safety, vulnerability, or SLSA-compliance claim. |
| `admission/release_source_v3.py` | Additive maintenance-aware Release Source Admission V3 manifest, cross-receipt byte binding, and independent signature domain. GitHub attestation `source_digest` remains the trusted workflow commit; the target source is retained in the exact V2 receipt subject. This module validates data only and does not contact GitHub, sign, or publish. |
| `admission/release_artifact_v2.py` | Additive Release Artifact Admission V2 manifest and cross-source byte binding for an exact V3 source admission and exact file digest. Builder/admitter replay is explicit and its provider source remains the trusted workflow commit. It has no workflow, provider invocation, signing, release, or deployment capability. |
| `admission/decision_envelope.py` | `v4.6.0` library-only Admission Decision Envelope V1 wire owner. It validates and canonicalizes one closed in-toto Statement projection for the Agent Change `ALLOW` profile. Structural inspection is non-authoritative; every external-action flag is fixed to false. |
| `admission/decision_sources.py` | Proof-family authority adapter for Admission Decision Envelope V1. It snapshots the exact Agent Change `.evb`, runs the existing strict finalizer/raw-Git/authorization verifier against external roots and expectations, then projects or literally reprojects the envelope. It introduces no key domain, CLI, workflow, merge, publication, or deployment operation. |
...
| `cli/preflight_commands.py` | Repository-aware CLI sequencing for static preflight: trusted config/override resolution, container defaults, verifier-pack path/digest validation, typed text/JSON projection, and deterministic exits. It injects effects into `policy/preflight.py` and never invokes the candidate test command. |
| `cli/init_command.py` | Stdlib-only, dependency-injected owner for credential-name validation, byte-exact blocking/advisory workflow generation, policy-path inference, and initialization write sequencing. The public facades inject callable providers so the owner retains every live path/filesystem/JSON lookup, including the historical callable-before-argument evaluation order, plus the historical helper names and output contract. |
...
| `cli/record_commands.py` | Dependency-injected orchestration for the Apache-core record family: exact-byte signature verification, semantic record verification, and ordered offline bundle verification. It performs no domain imports or direct filesystem/crypto effects; `cli/__init__.py` injects the established entry-time snapshots and live compatibility seams. |
| `cli/evidence_sealing_commands.py` | Dependency-injected orchestration for the platform sealing family: validated evidence-envelope creation (`bundle-evidence`) and trusted record finalization (`finalize-record`), extracted verbatim from the record owner so the open-core license boundary is also a module boundary. Same zero-effect, facade-injected contract. |
