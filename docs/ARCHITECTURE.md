...
| `verifiers/repo_verifier.py` | **The compatibility orchestrator.** Coordinates repository admission/materialization, sticky evidence, phase composition/final projection, and workspace cleanup. It injects live snapshot/runtime-identity providers into the two bounded continuity owners. For container runs it resolves a fresh canonical immutable image ID per verification and keeps that pin context-local across setup, suite, and pack phases. |
