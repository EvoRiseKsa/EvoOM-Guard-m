...
| `workspace/` | Contained workspace I/O: atomic descriptor-relative/no-follow operations on POSIX; reparse rejection plus pre/post parent/object identity checks as a non-atomic Windows fallback. `candidate_tree.py` owns bounded base/head intake, while `repository.py` owns filtered, symlink-preserving repository copying and cleanup exception precedence. Windows repository copying rejects observed junctions/non-symlink reparse objects but still requires a quiescent source; it is not an atomic snapshot. Historical `repo_verifier` facades preserve their API and live monkeypatch seams. |
