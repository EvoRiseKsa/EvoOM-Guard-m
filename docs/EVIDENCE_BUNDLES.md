...
Install the signing extra from ledger-recorded release `v4.6.0` and generate
an Ed25519 key once:

```bash
pip install "evoom-guard[sign] @ git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v4.6.0"
evo-guard keygen --key judge.pem --pub judge.pub
```
<!-- END EVOGUARD_PROJECT_STATUS:EVIDENCE_BUNDLES_RELEASE_PIN -->

The pinned `v4.6.0` release supports both the frozen schema-1.11 bundle
...
In `v4.6.0`, key generation uses the same fail-closed
...
In `v4.6.0`, the command strict-parses and
