...
`v4.4.2`, `v4.5.0`, and `v4.6.0` protected A–H ledgers, and as the minimum gate for later releases
...
7. **Immutable release artifact controls**
   ...
   - For each SBOM-enabled release, including `v4.4.2`, `v4.5.0`, and `v4.6.0`, require exactly the
...
The signed [`v4.4.2` ledger](../evidence/release-ledgers/v4.4.2/RELEASE_LEDGER.json),
[`v4.5.0` ledger](../evidence/release-ledgers/v4.5.0/RELEASE_LEDGER.json),
and latest [`v4.6.0` ledger](../evidence/release-ledgers/v4.6.0/RELEASE_LEDGER.json)
record completed operations under this contract. The checks remain normative
for later releases; their future-tense instructions must not be read as denying
any retained evidence set.
...
- `tests/baseline/schema/release-ledger-v2.schema.json` is the contract used by
  the signed `v4.4.2`, `v4.5.0`, and `v4.6.0` post-publication protected A-H ledgers and later v2
...
- Before a v2 ledger is committed, run:
  ...
  For every v2 ledger, including `v4.6.0`, repeat this validation against the
  bytes committed to protected main before retiring its publication authority.
...
- The two admission signing-secret names have post-H, successful, fully
  ...
  `v4.4.2`, `v4.5.0`, and `v4.6.0` ledgers record publication deploy-key retirement as
