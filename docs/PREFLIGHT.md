...
Markdown evidence. A separate completeness step requires both files to be
non-empty; missing or empty JSON/Markdown evidence makes the job red.
Checkout/setup failures, Action crashes, or upload failures can also make the
job red. It has read-only permissions,
