...
than claiming an atomic snapshot. `repo_verifier` retains call-time facades, so
Guard, black-box, evidence, and monkeypatch-based adopters keep the same
callable identities and dynamic effect seams.
