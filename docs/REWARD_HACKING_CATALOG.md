...
| 11b | **Neuter assertions** — candidate-imported source monkeypatches `unittest.TestCase.assert*` to no-ops, so the repo suite honestly reports all-pass | same in-process root cause as 11a; the report is truthful, so tamper detection has nothing to catch. Closed today by a judge-owned verifier pack whose assertions use bare `assert` (a repo-native assertion-liveness probe is planned) | ⚠️ false `PASS` (`known_gap`) in the repo-native suite | `tests/test_report_integrity.py::InProcessForgeryIsTheKnownBoundary::test_assertion_neutering_currently_yields_a_false_pass` |
