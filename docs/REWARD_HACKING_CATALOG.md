...
| 11b | **Neuter assertions** — candidate-imported source monkeypatches `unittest.TestCase.assert*` to no-ops, so the repo suite honestly reports all-pass | same in-process root cause as 11a; the report is truthful, so tamper detection has nothing to catch. Closed by `guard --require-assert-liveness` (auto-injects a judge-owned canary plugin into the pytest command; pytest-only, tamper-proof under `--isolation docker`), by a judge-owned verifier pack whose assertions use bare `assert`, **or** by appending the shipped canary (`benchmarks/self_proof/assert_liveness_canary.py`) to the test command — all probe the neutable methods in-session and enforce with a bare `assert` | ⚠️ false `PASS` (`known_gap`) in the plain default profile; 🚨 `TAMPERED` (`assertion_liveness_failed`) under `--require-assert-liveness` (the appended canary / pack yield `FAIL`); a genuine break stays a plain `FAIL` | `tests/test_report_integrity.py::RequireAssertLivenessClosesNeutering::test_the_flag_turns_a_neuter_false_pass_into_a_tamper`, `::test_a_genuine_break_stays_a_plain_fail_not_a_tamper`, `::InProcessForgeryIsTheKnownBoundary::test_assertion_neutering_currently_yields_a_false_pass`, `::test_the_shipped_canary_appended_closes_assertion_neutering`; proof `benchmarks/self_proof/assert_liveness.py` |
| 11c | **Rewrite a protected judging file mid-run** — candidate-imported source rewrites a protected test at collection time (pre-gate saw it pristine) | the protected-path gate is pre-execution; caught only by after-suite tree-continuity | ⚠️ false `PASS` by default; 🚨 `TAMPERED` (`suite_drift`) under `--require-suite-continuity` | `tests/test_report_integrity.py::SuiteContinuityClosesMidRunRewrite` |
