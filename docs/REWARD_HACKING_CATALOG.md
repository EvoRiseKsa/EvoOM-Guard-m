...
| 11b | **Neuter assertions** — candidate-imported source monkeypatches `unittest.TestCase.assert*` to no-ops, so the repo suite honestly reports all-pass | same in-process root cause as 11a; the report is truthful, so tamper detection has nothing to catch. Closed by a judge-owned verifier pack whose assertions use bare `assert`, **or** by appending the shipped assertion-liveness canary (`benchmarks/self_proof/assert_liveness_canary.py`) to the test command — it probes the neutable methods in-session and enforces with a bare `assert` | ⚠️ false `PASS` (`known_gap`) in the pack-less/canary-less repo suite; ❌ `FAIL` once the canary is appended | `tests/test_report_integrity.py::InProcessForgeryIsTheKnownBoundary::test_assertion_neutering_currently_yields_a_false_pass`, `::test_the_shipped_canary_appended_closes_assertion_neutering`; proof `benchmarks/self_proof/assert_liveness.py` |
