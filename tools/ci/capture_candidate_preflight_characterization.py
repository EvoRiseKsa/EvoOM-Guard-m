"""Check or explicitly replace the reviewed candidate-preflight vector."""
...
from candidate_preflight_characterization_harness import (  # noqa: E402
    canonical_json,
    capture_all,
)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="evo-candidate-preflight-") as temp:
    ...
    if VECTOR.read_text(encoding="utf-8") != current:
        print(
            "Candidate-preflight characterization differs; "
            "run pytest for a case diff.",
            file=sys.stderr,
        )
        return 1
    print("Candidate-preflight characterization matches the frozen vector.")
    return 0
