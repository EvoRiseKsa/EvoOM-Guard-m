"""Capture/check the frozen verifier-pack characterization vector."""
...
from repo_pack_characterization_harness import (  # noqa: E402
    SCHEMA_VERSION,
    canonical_json,
    capture_all,
)

VECTOR = TESTS / "fixtures" / "refactor-safety" / "repo-pack-v1.json"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="evoguard-repo-pack-characterization-") as temp:
    ...
    if VECTOR.read_text(encoding="utf-8") != rendered:
        print(
            "Verifier-pack characterization drifted; run with --write only after review.",
            file=sys.stderr,
        )
        return 1
    print("Verifier-pack characterization matches the frozen vector.")
    return 0
