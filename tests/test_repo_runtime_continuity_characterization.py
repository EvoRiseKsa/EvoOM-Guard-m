"""Frozen equivalence gates for repository runtime continuity."""
...
import pytest
from repo_runtime_continuity_characterization_harness import (
    CASE_NAMES,
    SCHEMA_VERSION,
    canonical_json,
    capture_case,
...
def test_repo_runtime_continuity_vector_metadata_is_exact() -> None:
    frozen = _frozen()
...
def test_frozen_repo_runtime_continuity_behavior(
    case_name: str,
    tmp_path: Path,
) -> None:
    ...
    if observed != expected:
        pytest.fail(
            "repository runtime-continuity behavior drifted:\n"
