"""Frozen wire-level equivalence seam for assurance extraction."""
...
import pytest
from assurance_characterization_harness import (
    CASE_NAMES,
    NORMALIZED_FIELDS,
    SCHEMA_VERSION,
    canonical_json,
...
VECTOR = Path(__file__).parent / "fixtures" / "refactor-safety" / "assurance-v1.json"
...
def test_assurance_characterization_metadata_is_exact() -> None:
    frozen = _frozen()
...
def test_frozen_assurance_payloads_and_diagnostics(case_name: str) -> None:
    ...
    if actual != expected:
        ...
        pytest.fail("assurance payload or diagnostic drifted:\n" + diff)
